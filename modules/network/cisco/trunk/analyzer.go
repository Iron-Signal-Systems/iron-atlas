...
func Classify(iface common.Interface) common.Classification {
	result := common.Classification{Class: common.ClassUnknown, EndpointAttribution: false}
	modeAdmin := strings.ToLower(strings.TrimSpace(iface.AdministrativeMode))
	modeOperational := strings.ToLower(strings.TrimSpace(iface.OperationalMode))
	...
	case strings.Contains(modeAdmin, "access") || strings.Contains(modeOperational, "access"):
		result.Class = common.ClassAccessEndpoint
		result.EndpointAttribution = true
		if strings.Contains(modeAdmin, "access") {
			result.Reasons = append(result.Reasons, "configured_access")
		}
		if strings.Contains(modeOperational, "access") {
			result.Reasons = append(result.Reasons, "operational_access")
		}
	default:
		result.Reasons = append(result.Reasons, "insufficient_access_evidence")
	}
	return result
}
...
func Analyze(iface common.Interface) []Finding {
	...
	if classification.Class != common.ClassInfrastructureTrunk {
		return nil
	}
